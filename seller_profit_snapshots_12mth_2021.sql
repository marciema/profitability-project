DROP TABLE IF EXISTS app_risk.app_risk.hist_seller_profit_calculation_drv01_loading_2021 CASCADE;	
	DROP TABLE IF EXISTS app_risk.app_risk.hist_seller_profit_calculation_drv02_loading_2021 CASCADE;
	DROP TABLE IF EXISTS app_risk.app_risk.hist_seller_profit_calculation_drv03_loading_2021 CASCADE;
	DROP TABLE IF EXISTS app_risk.app_risk.hist_seller_profit_calculation_drv04_loading_2021 CASCADE;
	DROP TABLE IF EXISTS app_risk.app_risk.hist_seller_profit_calculation_drv04_pd1_loading_2021 CASCADE;
	DROP TABLE IF EXISTS app_risk.app_risk.hist_seller_profit_calculation_drv04_pd2_loading_2021 CASCADE;
	DROP TABLE IF EXISTS app_risk.app_risk.hist_seller_profit_calculation_drv04_pd3_gpv_loading_2021 CASCADE;
	DROP TABLE IF EXISTS app_risk.app_risk.hist_seller_profit_calculation_drv04_pd4_loading_2021 CASCADE;
	DROP TABLE IF EXISTS app_risk.app_risk.hist_seller_profit_calculation_drv05_pd_loading_2021 CASCADE;
	DROP TABLE IF EXISTS app_risk.app_risk.hist_seller_profit_calculation_drv05_lgd_loading_2021 CASCADE;
	DROP TABLE IF EXISTS app_risk.app_risk.hist_seller_profit_calculation_drv05_presale_loading_2021 CASCADE;
	DROP TABLE IF EXISTS app_risk.app_risk.hist_seller_profit_calculation_drv06_loss_loading_2021 CASCADE;
    DROP TABLE IF EXISTS app_risk.app_risk.hist_seller_profit_calculation_drv07_loading_2021 CASCADE;
	
	--create app_risk.app_risk.historical cohort table
	CREATE OR REPLACE TABLE app_risk.app_risk.hist_seller_profit_calculation_cohort_2021
	(
	cohort_date DATE
	);
	
	SELECT * FROM app_risk.app_risk.hist_seller_profit_calculation_cohort_2021;
	
	INSERT INTO app_risk.app_risk.hist_seller_profit_calculation_cohort_2021 (cohort_date)
	VALUES
	('2021-01-04'),
	('2021-01-11'),
	('2021-01-18'),
	...
	CREATE OR REPLACE TABLE app_risk.app_risk.hist_seller_profit_calculation_drv01_loading_2021 AS
	SELECT
	gpv.user_token
	,gpv.cohort_date
	,gpv.currency_code
	...
	FROM app_risk.app_risk.hist_seller_profit_calculation_cohort_2021 ct
	...
	CREATE OR REPLACE TABLE app_risk.app_risk.hist_seller_profit_calculation_drv02_loading_2021 AS
	SELECT r.user_token
	,ct.cohort_date
	,r.product_category
	,du.is_unit
	...
	CREATE OR REPLACE TABLE app_risk.app_risk.hist_seller_profit_calculation_drv03_loading_2021 AS
	SELECT gpv.*
	...
	,mer.unit_cnt
	FROM app_risk.app_risk.hist_seller_profit_calculation_drv01_loading_2021 gpv
	LEFT OUTER JOIN
	(SELECT user_token
	,cohort_date
	...
	FROM app_risk.app_risk.hist_seller_profit_calculation_drv02_loading_2021
	WHERE is_unit = 1
	GROUP BY 1,2
	...
	,COUNT(*) AS unit_cnt
	FROM app_risk.app_risk.hist_seller_profit_calculation_drv01_loading_2021
	GROUP BY 1,2
	...
	CREATE OR REPLACE TABLE app_risk.app_risk.hist_seller_profit_calculation_drv04_loading_2021 AS
	SELECT
	user_token
	,cohort_date
	,currency_code
	...
	,unit_cnt
	FROM app_risk.app_risk.hist_seller_profit_calculation_drv03_loading_2021
	;
	
	-----------05.Pull the PD calibration in the past 12 months--------------------
	CREATE OR REPLACE TABLE app_risk.app_risk.hist_seller_profit_calculation_drv04_pd1_loading_2021 AS
	...
	GROUP BY 1,2
	;
	CREATE OR REPLACE TABLE app_risk.app_risk.hist_seller_profit_calculation_drv04_pd2_loading_2021 AS
	SELECT fpt.unit_token
	,u.cohort_date
	,avg(rs.score) AS pd_score
	FROM app_risk.app_risk.hist_seller_profit_calculation_drv04_pd1_loading_2021 u
	...
	GROUP BY 1,2
	;
	CREATE OR REPLACE TABLE app_risk.app_risk.hist_seller_profit_calculation_drv04_pd3_gpv_loading_2021 AS
	SELECT
	vf.user_token
	,ct.cohort_date
    ...
    CREATE OR REPLACE TABLE app_risk.app_risk.hist_seller_profit_calculation_drv04_pd4_sqrr_loading_2021 AS
SELECT  
...
CREATE OR REPLACE TABLE app_risk.app_risk.hist_seller_profit_calculation_drv05_pd_loading_2021 AS
...
FROM app_risk.app_risk.hist_seller_profit_calculation_drv04_pd2_loading_2021 drv 
INNER JOIN app_risk.app_risk.hist_seller_profit_calculation_drv04_pd3_gpv_loading_2021 gpv ON drv.unit_token = gpv.user_token and drv.cohort_date = gpv.cohort_date
INNER JOIN app_risk.app_risk.hist_seller_profit_calculation_drv04_loading_2021 bu ON drv.unit_token = bu.user_token and drv.cohort_date = bu.cohort_date
LEFT OUTER JOIN app_risk.app_risk.hist_seller_profit_calculation_drv04_pd4_sqrr_loading_2021 sqrr ON drv.unit_token = sqrr.user_token and drv.cohort_date = sqrr.cohort_date
;
    /*FROM app_risk.app_risk.hist_seller_profit_calculation_drv04_pd2_loading_2021 drv
	INNER JOIN app_risk.app_risk.hist_seller_profit_calculation_drv04_pd3_gpv_loading_2021 gpv ON drv.unit_token = gpv.user_token and drv.cohort_date = gpv.cohort_date
	;*/
    
	SELECT COUNT(*), COUNT(distinct unit_token, cohort_date) FROM app_risk.app_risk.hist_seller_profit_calculation_drv05_pd_loading_2021;
   
	-----------06.Pull the LGD--------------------
	CREATE OR REPLACE TABLE app_risk.app_risk.hist_seller_profit_calculation_drv05_lgd_loading_2021 AS
	SELECT fpt.unit_token
	,u.cohort_date
...
CREATE OR REPLACE TABLE app_risk.app_risk.hist_seller_profit_calculation_drv05_presale_loading_2021 AS
...
FROM app_risk.app_risk.hist_seller_profit_calculation_drv04_loading_2021 drv
...
CREATE OR REPLACE TABLE app_risk.app_risk.hist_seller_profit_calculation_drv06_loss_loading_2021 AS
select
cb.user_token,
ct.cohort_date,
...
CREATE OR REPLACE TABLE app_risk.app_risk.hist_seller_profit_calculation_drv07_loading_2021 AS
SELECT 
r.user_token
,r.cohort_date
,r.currency_code
...
FROM app_risk.app_risk.hist_seller_profit_calculation_drv04_loading_2021 r
LEFT OUTER JOIN app_risk.app_risk.hist_seller_profit_calculation_drv05_pd_loading_2021 pd ON r.user_token = pd.unit_token and r.cohort_date = pd.cohort_date
LEFT OUTER JOIN app_risk.app_risk.hist_seller_profit_calculation_drv05_lgd_loading_2021 lgd ON r.user_token = lgd.unit_token and r.cohort_date = lgd.cohort_date
LEFT OUTER JOIN app_risk.app_risk.hist_seller_profit_calculation_drv05_presale_loading_2021 presale ON r.user_token = presale.user_token and r.cohort_date = presale.cohort_date
LEFT OUTER JOIN app_risk.app_risk.hist_seller_profit_calculation_drv04_pd3_gpv_loading_2021 gpv ON r.user_token = gpv.user_token and r.cohort_date = gpv.cohort_date
LEFT OUTER JOIN app_risk.app_risk.hist_seller_profit_calculation_drv06_loss_loading_2021 cb ON r.user_token = cb.user_token and r.cohort_date = cb.cohort_date
;


	CREATE TABLE IF NOT EXISTS app_risk.app_risk.seller_profit_snapshots_12mth_2021 lIKE app_risk.app_risk.hist_seller_profit_calculation_drv07_loading_2021;
	ALTER TABLE app_risk.app_risk.seller_profit_snapshots_12mth_2021 SWAP WITH app_risk.app_risk.hist_seller_profit_calculation_drv07_loading_2021;
